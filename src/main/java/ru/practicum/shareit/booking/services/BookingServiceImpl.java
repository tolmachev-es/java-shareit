...
public class BookingServiceImpl implements BookingService {
        ...
        if (item.getAvailable()) {
            ...
            return BookingMapper.BOOKING_MAPPER.toDto(
                    BookingMapper.BOOKING_MAPPER.fromEntity(
                            bookingRepo.save(createBooking)));
        } else {
    ...
    public BookingDtoResponse approve(Long bookingId, Long userId, boolean status) {
        Optional<BookingEntity> findBooking = bookingRepo.findById(bookingId);
        if (findBooking.isPresent()) {
            if (findBooking.get().getItem().getOwner().getId().equals(userId)) {
                ...
                return BookingMapper.BOOKING_MAPPER.toDto(
                        BookingMapper.BOOKING_MAPPER.fromEntity(
                                bookingRepo.save(findBooking.get())));
            } else {
    ...
    public Set<BookingDtoResponse> getByBooker(String bookingState, Long userId, Integer from, Integer size) {
        ...
        switch (bookingStateEnum) {
            case ALL:
                result = bookingRepo.findBookingEntitiesByBooker_IdOrderByIdDesc(
                        userId, pageable).toSet();
                break;
            case PAST:
                result = bookingRepo.findBookingEntitiesByBooker_IdAndEndBefore(
                        userId, LocalDateTime.now(), pageable).toSet();
                break;
            case FUTURE:
                result = bookingRepo.findBookingEntitiesByBooker_IdAndStartAfter(
                        userId, LocalDateTime.now(), pageable).toSet();
                break;
            case CURRENT:
                result = bookingRepo.findBookingEntitiesByBooker_IdAndStartBeforeAndEndIsAfter(
                        userId, LocalDateTime.now(), LocalDateTime.now(), pageable).toSet();
                break;
            case WAITING:
                result = bookingRepo.findBookingEntitiesByBooker_IdAndStatus(
    ...
    public Set<BookingDtoResponse> getByOwner(String bookingState, Long userId, Integer from, Integer size) {
        ...
        switch (bookingStateEnum) {
            case ALL:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdOrderByIdDesc(
                        userId, pageable).toSet();
                break;
            case PAST:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdAndEndBefore(
                        userId, LocalDateTime.now(), pageable).toSet();
                break;
            case FUTURE:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdAndStartAfter(
                        userId, LocalDateTime.now(), pageable).toSet();
                break;
            case CURRENT:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdAndStartBeforeAndEndAfter(
                        userId, LocalDateTime.now(), LocalDateTime.now(), pageable).toSet();
                break;
            case WAITING:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdAndStatus(
                        userId, BookingStatus.WAITING, pageable).toSet();
                break;
            case REJECTED:
                result = bookingRepo.findBookingEntitiesByItem_OwnerIdAndStatus(
